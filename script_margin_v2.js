...
const children = [
  ...
  new Paragraph({
    ...
    children: [
      new TextRun({ text: 'v2修正：①書き込み問題1文対応　②探索パート短文連打化　③繋がり根拠補強　④エマの強さ追加　⑤行動に間を入れる', font: 'Arial', size: 20, color: 'C0392B' }),
    ]
  }),
  ...
  blank(),
  p('エマには、それがなかった。'),
  p('だからこそ、一冊の本を誰よりも丁寧に、最後まで読んだ。', { bold: true }), // ← 修正④：強さを示す
  blank(),
  p('大学院の最初の1年は、ただ生き延びることだった。'),
  p('毎晩図書館に残り、閉館ギリギリまで読み続けた。'),
  ...
  p('「ここは読まなくていい。試験範囲外」'),
  blank(),
  p('一部の文字は薄く消された跡があった。', { color: '666666' }), // ← 修正①：書き込み違反問題への対応
  p('おそらく、誰かが返却前に消そうとしたのだろう。', { color: '666666' }),
  p('それでも、まだ読めた。', { color: '666666' }),
  blank(),
  p('誰かが、丁寧に道を作っていた。'),
  blank(),
  ...
  p('結果が出た。合格だった。'),
  blank(),
  p('翌日、エマは図書館に向かった。', { italic: true }), // ← 修正⑤：「走った」→翌日に間を入れる
  p('あの本を抱えて、受付に向かった。'),
  ...
  p('大学のコミュニティフォーラムに書き込み、卒業生グループにも転送してもらった。'), // ← 修正③：繋がり根拠補強
  blank(),
  p('通知を確認した。何もなかった。'),  // ← 修正②：短文連打化
  p('また確認した。何もなかった。'),
  p('また確認した。'),
  blank(),
  p('数週間が過ぎた。'),
  ...
  blank(),
  p('サラ・トンプソン。'),
  p('数年前に同じ課程を修了し、今は地方の法律援助機関で働いているという。'),
  blank(),
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync('/mnt/user-data/outputs/台本_余白の文字_v2.docx', buf);
  console.log('done');
});
